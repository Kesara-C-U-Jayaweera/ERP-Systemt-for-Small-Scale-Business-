...
export function InvoiceClient({ products }: { products: Product[] }) {
  ...
  useBarcodeScanner({
    onScan: (barcode) => {
      ...
      if (product) {
        // play simple beep
        const audio = new Audio("data:audio/wav;base64,UklGRl9vT19XQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YU") // extremely short dummy base64 beep
        audio.play().catch(() => {})
        ...
        setLines(prev => {
          ...
          if (existingIdx >= 0) {
            const newLines = [...prev]
            newLines[existingIdx] = { ...newLines[existingIdx], qty: newLines[existingIdx].qty + 1 }
            return newLines
          }
          // Otherwise add new line
          // If first line is totally empty, overwrite it
  ...
  function updateLine(index: number, field: keyof LineItem, value: string | number) {
    setLines(lines.map((l, i) => {
      ...
      // Auto-fill if they type an exact product name (basic combobox-like behavior for now)
      if (field === "description" && typeof value === "string") {
        const found = products.find(p => p.name.toLowerCase() === value.toLowerCase())
        if (found) {
          updated.productId = found.id
          updated.unitPrice = Number(found.price)
        } else {
    ...
    <div className="flex flex-col gap-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Create an invoice and scan barcodes to add items instantly.</p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-3">
            <CardTitle className="text-base">Line Items (Scan Barcode to Add)</CardTitle>
            <Button type="button" variant="outline" size="sm" onClick={addLine}>+ Add Line</Button>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="grid grid-cols-12 gap-2 text-xs font-semibold text-muted-foreground px-1">
              <span className="col-span-4">Item (Scan or Type)</span>
            ...
            {lines.map((line, i) => (
              <div key={i} className="grid grid-cols-12 gap-2 items-center">
                ...
                <Input className="col-span-2 text-right" type="number" min="1" value={line.qty}
                  onWheel={(e) => e.currentTarget.blur()}
                  onChange={e => updateLine(i, "qty", Number(e.target.value))} />
                <Input className="col-span-2 text-right" type="number" step="0.01" min="0" value={line.unitPrice}
                  onWheel={(e) => e.currentTarget.blur()}
                  onChange={e => updateLine(i, "unitPrice", Number(e.target.value))} />
                <Input className="col-span-2 text-right" type="number" step="0.01" min="0" max="100" value={line.taxRate || ""} placeholder="0"
                  onWheel={(e) => e.currentTarget.blur()}
